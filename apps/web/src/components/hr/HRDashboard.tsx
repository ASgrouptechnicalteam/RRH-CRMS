...
const TeamPerformanceTab: React.FC = () => {
  ...
  const columns: ColumnDef<TeamPerformanceRow>[] = [
    ...
    },
    {
      ...
      render: (r) => (
        <span
          className={`px-2 py-0.5 rounded text-[11px] font-bold ${r.conversionRate >= 10 ? 'bg-success-100 text-success-800' : 'bg-slate-100 text-slate-700'}`}
        >
...
export const HRDashboard: React.FC = () => {
  ...
  const [activeTab, setActiveTab] = useState<
    'OVERVIEW' | 'DIRECTORY' | 'HOLIDAYS' | 'PERFORMANCE' | 'DOCUMENTS'
  >((searchParams.get('tab') as any) || 'DIRECTORY');
  ...
  useEffect(() => {
    ...
    const fetchHrMetrics = async () => {
      ...
      try {
        const res = await fetchWithAuth(`${API_BASE_URL}/analytics/hr-overview`);
        if (res.ok) {
          const data = await res.json();
          setHrMetrics({
            headcount: data.headcount || 0,
            newHiresThisMonth: data.newHiresThisMonth || 0,
            leavesToday: data.onLeaveToday || 0,
            leavesYesterday: data.onLeaveYesterday || 0,
            avgConversion: data.conversionThisMonth || 0,
            avgConversionLastMonth: data.conversionLastMonth || 0,
          });
        }
      } catch (err) {
  ...
  const leaveDelta = hrMetrics.leavesToday - hrMetrics.leavesYesterday;
  const leaveTrend =
    leaveDelta > 0
      ? { direction: 'down' as const, value: `+${leaveDelta}`, label: 'More than yesterday' }
      : {
          direction: 'up' as const,
          value: leaveDelta === 0 ? 'No change' : `${leaveDelta}`,
          label: 'vs yesterday',
        };

  const conversionDelta = hrMetrics.avgConversion - hrMetrics.avgConversionLastMonth;
  const conversionTrend = {
    direction: (conversionDelta >= 0 ? 'up' : 'down') as 'up' | 'down',
    value: `${conversionDelta >= 0 ? '+' : ''}${conversionDelta.toFixed(1)}%`,
    label: 'vs last month',
  };
    ...
    <div className="space-y-6">
      ...
      <div className="bg-gradient-to-r from-navy-900 via-navy-800 to-slate-900 rounded-3xl p-6 text-white shadow-xl border border-navy-700/30">
        ...
        <p className="text-sm text-navy-200/80 max-w-2xl">
          Manage the organizational directory, approve leave requests, track daily attendance, and
          correlate CRM performance to personnel.
        </p>
      </div>
          ...
          <div className="space-y-6 animate-fadeIn">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
              <StatCard
                ...
                value={metricsLoading ? '...' : hrMetrics.headcount.toString()}
                icon={Users}
                trend={{
                  direction: 'up',
                  value: `+${hrMetrics.newHiresThisMonth}`,
                  label: 'New this month',
                }}
              />
              <StatCard
                label="On Leave Today"
                value={metricsLoading ? '...' : hrMetrics.leavesToday.toString()}
                icon={ShieldAlert}
                trend={metricsLoading ? undefined : leaveTrend}
              />
              <StatCard
                label="Avg Pipeline Conversion"
                value={metricsLoading ? '...' : `${hrMetrics.avgConversion.toFixed(1)}%`}
                icon={TrendingUp}
                trend={metricsLoading ? undefined : conversionTrend}
              />
            </div>
            <div className="bg-surface border border-slate-200 rounded-xl p-8 text-center text-slate-500">
              <p className="text-sm font-semibold mb-2">
                Extended HR Analytics arriving in a future update.
              </p>
              <p className="text-xs">
                Navigate to the Directory, Attendance, or Performance tabs for actionable data.
              </p>
            </div>
          </div>
        )}
        ...
        {activeTab === 'DOCUMENTS' && (
          <div className="bg-surface border border-slate-200 rounded-xl p-12 text-center flex flex-col items-center justify-center animate-fadeIn min-h-[300px]">
            ...
            <p className="text-sm text-slate-500 max-w-md">
              Secure onboarding documents, contracts, and ID proofs storage will be available in the
              upcoming Document Management Phase.
            </p>
          </div>
        )}
      </div>
