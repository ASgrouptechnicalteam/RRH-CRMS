...
export const PMBlindApprovalQueue: React.FC = () => {
  ...
  const fetchQueue = async () => {
    ...
    try {
      ...
      if (empRes.ok) {
        // Only PMs and Agents can be reassignment targets per policy
  ...
  const handleReassignSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const res = await fetchWithAuth(`${API_BASE_URL}/site-visits/${selectedVisitId}/reassign`, {
        ...
        body: JSON.stringify({
          assigned_to_id: parseInt(reassignTargetId, 10),
      ...
      const data = await res.json();
      if (res.ok) {
        showToast('Site visit reassigned successfully', 'success');
  ...
  if (isLoading) {
    return <div className="p-8 text-center text-slate-500">Loading Blind Approval Queue...</div>;
  }
        ...
        <div className="fixed inset-0 z-50 bg-slate-900/50 backdrop-blur-sm flex items-center justify-center p-4">
          <div className="bg-white rounded-3xl shadow-xl w-full max-w-md overflow-hidden">
            <div className="px-6 py-4 border-b border-slate-100 flex items-center justify-between">
              <h3 className="text-lg font-bold text-navy-900">Route Visit</h3>
              <button
