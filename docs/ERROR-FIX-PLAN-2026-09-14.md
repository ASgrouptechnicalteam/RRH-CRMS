...
- **Still open — DB-level constraint:** did **not** touch the schema or write a `prisma migrate` SQL file yet. A straight `UNIQUE(company_id, phone)` migration is unsafe to ship blind: (a) existing rows use mixed phone spellings, so a raw-string unique index would miss cross-format duplicates and (b) I don't know whether _production_ currently holds real duplicates that would make the migration fail on deploy (this repo deploys migrations manually via `prisma migrate deploy`, per `.env`, so nothing here reaches production without you running that). Added `apps/api/src/scripts/normalizeEmployeeContacts.ts` instead: a standalone, idempotent script — dry-run by default, `--apply` to write — that (1) rewrites existing phone/email values to the canonical form the app now uses and (2) reports any `(company_id, phone/email)` pairs still duplicated afterward, which must be resolved by hand before a unique index is safe to add. Verified dry-run, apply, and idempotent re-run against the local DB.
- **Blocking question for you:** how do you want the production duplicate check done — (a) I run `normalizeEmployeeContacts.ts` (dry-run first) against production myself, (b) you run it and share the report, or (c) skip the DB constraint for now and rely on the app-layer check alone (already closes the reported bug; the constraint is defense-in-depth for anything that writes to `Employee` outside these three routes, e.g. bulk scripts)? Once duplicates (if any) are resolved, adding the `@@unique` migration is a 10-minute follow-up.
