...
### 1.4 `[x]` Site visits, demos, interests, opportunities
...
- **Done 2026-09-14:** Unlike 1.3 (mostly already built), this gap was real and full. `SiteVisitCreateSchema`/`RescheduleSchema`/`OutcomeSchema` now accept `project_unit_ids`/`project_unit_id` alongside the property fields (XOR, same compound-unique-key pattern as `LeadPropertyInterest` for the `SiteVisitProperty` join rows). `resolveVisitProject`, `bookVisit`, `rescheduleVisit`, and `completeVisit` all rewritten to handle both. `OpportunityService.createFromLead`/`updateOpportunity`/`getOpportunityById`/`getOpportunitiesByLead`/`convertToBooking` all now accept and surface `project_unit_id` (XOR with `property_id`, switching target clears the other). `LeadDetailModal.tsx`'s "Attach Property" visit-scheduling dropdown and `SalesOpportunityDetails.tsx`'s deal-target dropdown both now list saved property _and_ unit interests in grouped `<optgroup>`s, matching the picker style already used in `CreateBookingModal.tsx`. `SiteVisitManagement.tsx`, `PMBlindApprovalQueue.tsx`, and `ActiveSiteVisitsBanner.tsx` render unit-linked visits with project/unit info instead of blank/"Unknown Property".
- **`routes/opportunities.ts` had a real pre-existing gap, found and fixed:** `OpportunityCreateSchema`/`OpportunityUpdateSchema` were imported but never wired into `validateRequestBody` on either the `POST /` or `PATCH /:id` route — both took fully unvalidated `req.body` (no XOR check, no type coercion, arbitrary extra fields passed straight to the service layer). Now wired; confirmed via a live request that a body with both `property_id` and `project_unit_id` set is now rejected 400 where it previously would have hit the database.
- **`LeadDetailModal.tsx`'s "Attach Property" dropdown had a real pre-existing bug, found and fixed:** it sent a bare `property_id` field, but `SiteVisitCreateSchema` only ever declared the plural `property_ids` — zod's default unknown-key stripping silently dropped the field, so "Attach Property" had never actually worked in that modal, for any lead, ever. Fixed by sending the plural array fields the schema actually validates.
- **Three more real, pre-existing bugs found while verifying the unit path end-to-end** (all blocking real leads generally, not just unit-targeted ones in the third case):
  1. `lead.workflow.ts`'s `BOOKING_INITIATED` guard checked `Opportunity.property_id` only, never `project_unit_id` — a unit-only Opportunity could never legally reach `BOOKING_INITIATED`, full stop.
  2. The `SITE_VISIT_COMPLETED → NEGOTIATION` guard's preview and the real §4 auto-create (`services/lead/status.ts`) both read the INTERESTED outcome's `.property` only, never `.project_unit` — completing a unit-only visit with an INTERESTED outcome could never advance the lead to Negotiation.
  3. The §4 auto-create's "find the INTERESTED outcome that unlocked this" query had **no `lead_id` scope at all** — it fetched whichever `INTERESTED` `SiteVisitProperty` row happened to be first in the entire table (any lead, any company), so any lead with older INTERESTED history elsewhere in the system silently got a blank Opportunity (no property/unit, no `expected_value`), permanently stuck.
- **Not built:** demo "interested properties" — `DemoInterestedProperty` is a Prisma model with **zero references anywhere in `apps/api/src`**. There's no existing feature here to extend; the plan's assumption that this flow exists was wrong. Flagging to you rather than building new feature work silently — say the word if you want it built.
- **Verified end-to-end against `test_db`, not just typechecked:** lead → unit interest → site visit booked/accepted/reconfirmed/rescheduled (still carrying the unit through every state)/confirmed/started/completed with a unit `INTERESTED` outcome → lead auto-advanced to `SITE_VISIT_COMPLETED` then `NEGOTIATION` → Opportunity auto-created correctly linked to the unit with the unit's own price as `expected_value` → manually advanced to `BOOKING_INITIATED` (previously impossible for a unit-only deal — bug #1 above) → `convert-to-booking` produced a real `Booking` row with `project_unit_id` set and `property_id` null. Also verified the XOR validation directly rejects a body with both ids (400, clear message) now that the routes are actually validated. Ran the full site-visit/opportunity regression suites (`opportunities-integration.test.ts`, `opportunities.test.ts`, `packet3-opp-booking.test.ts`, `phase4-site-visits.test.ts`, `site-visit-complete-lead-cascade.test.ts`, `site-visit-complete-no-property.test.ts`, `site-visit-feedback.test.ts`, `siteVisits.test.ts`) both with and without this item's changes (via `git stash`) to confirm the same 4 pre-existing failures are unaffected: `opportunities-integration.test.ts` tests 7/8/8b/12 (Task↔Opportunity association, unrelated to this item's code paths), `phase4-site-visits.test.ts`/`siteVisits.test.ts`'s "Cross-Company Actor blocked" test, and `opportunities.test.ts` failing to run at all standalone due to a missing `JWT_ACCESS_SECRET`/`JWT_REFRESH_SECRET` test-env config (fails identically before any of this item's code loads).

### 1.5 `[x]` Dashboards and widgets
...
- **Done 2026-09-15:** checked each of the four named files individually rather than blanket-switching to `/inventory` — most turned out to be genuinely property-scoped workflows with no unit/project equivalent to extend:
  - **`md/UnassignedPropertiesWidget.tsx` — the real gap, fixed.** `Project` has its own `assigned_pm_id`, entirely independent of `Property`'s (not inherited by its units — the PM a unit's site visits get routed to comes from the _Project_, per `resolveVisitProject` from item 1.4). This widget only ever queried unassigned standalone Properties, so an unassigned Project — and every unit inside it, left without a routed PM as a result — was invisible to the MD. Added `GET /api/v1/projects?unassigned=true` (mirrors Property's own filter) and the widget now fetches both lists, tags each row `PROPERTY`/`PROJECT`, and routes "Assign PM" to the matching endpoint.
  - **`PMDashboard.tsx`** — "Pending PM Verification" only ever queried `Property.status = PENDING_VERIFICATION`, a Property-only listing-pipeline stage. Project verification is a separate workflow (`Project.verification_status`, MD/Admin-only, submitted by PM directly on the project page) with no shared queue — renamed the stat to "Pending Property Verification" so it doesn't imply broader coverage than it has.
  - **`DigitalMarketingExecutiveDashboard.tsx` / `DigitalMarketingHeadDashboard.tsx`** — checked the schema: `dm_executive_id` and `PENDING_DM_POLISH` exist only on `Property`. There's no unit or project-level "content polish" step to extend this to — these dashboards are correctly property-scoped as-is, no change made.
  - **`ProjectDossier.tsx` — turned out to be dead code.** Grepped the entire frontend: nothing imports or renders it anywhere. The live `/projects/:id` page is `ProjectDashboard.tsx`, a different component that already correctly uses `listProjectUnits`/real `ProjectUnit` data for its own Units tab. `ProjectDossier.tsx`'s own "Units" tab reads legacy Property-based units, and its `BulkUnitWizard` posts to `POST /projects/:id/units/bulk` — a route that doesn't exist anywhere in the API (grepped `apps/api/src`, zero matches), so unit creation through this dead component would 404 even if it were somehow reachable. Left in place per the no-delete rule; flagging as dead/broken code rather than spending effort polishing something nothing renders.
- **Verified live, not just typechecked:** `GET /projects?unassigned=true` returned the 4 real unassigned test projects from `test_db`; the MD Executive Dashboard's widget rendered them correctly alongside the 5 unassigned properties (title, location, "(Project)" tag); used the widget's own "Assign PM" dropdown on "Phase12 Verify Meadows" and confirmed via `GET /projects/:id` that `assigned_pm_id` was persisted and the row dropped off the list. `project-workflow.test.ts` still green. Two other project test files fail (`public-project-api.test.ts`, `wr5-project-publication.test.ts`) — both a separate public/`x-api-key` endpoint this item never touches, confirmed pre-existing (zero uncommitted changes on either file, failures are 404s on an unrelated route).

### 1.6 `[x]` Project types list
...
- **Done 2026-09-15:** you confirmed the proposed list. Implemented with two adjustments made during the actual work:
  - **Spelling aligned to the existing Property category taxonomy**, not the plan doc's rough draft — `apps/web/src/components/properties/propertyWizardShared.tsx`'s `PROPERTY_CATEGORIES` already uses `COMMERCIAL_SHOP` (singular) and `AGRICULTURAL_LAND` (not `FARM_LAND`), so the new `ProjectType` values use those exact spellings instead of introducing a third, inconsistent naming variant — which was the whole point of "mirroring the property categories."
  - **`MIXED`/`COMMERCIAL` kept, not dropped.** An enum value already in use by an existing row can't be safely removed without a verified data migration, and production DB access is off-limits (standing instruction). `test_db` had zero rows using either value, but production wasn't checked (can't check it myself). Both stay valid in the DB enum and the zod schema; the create/edit wizard's picker only offers the new richer list going forward, and `ProjectManagement.tsx`'s filter/label map shows them as "Mixed (legacy)" / "Commercial (legacy)" so any existing row stays visible and filterable rather than silently disappearing.
  - **"Unit-type defaults per project type"** — checked the codebase for this; no such defaulting logic exists anywhere (grepped `unit_type.*default`/`defaultUnitType`, zero matches), so there was nothing to update. Not building it now — new logic to auto-suggest a `UnitType` from a chosen `ProjectType` would be new feature work, out of scope for a fix-the-enum item.
- Final `ProjectType` values: `PLOTTED`, `APARTMENT`, `VILLA`, `INDEPENDENT_HOUSE`, `ROW_HOUSE`, `AGRICULTURAL_LAND`, `FARM_HOUSE`, `COMMERCIAL_SHOP`, `COMMERCIAL_OFFICE`, `MIXED_RESIDENTIAL`, `MIXED_USE`, `TOWNSHIP`, `GATED_COMMUNITY`, plus legacy `MIXED`/`COMMERCIAL`, plus `OTHER`.
- Changed: `prisma/schema.prisma` enum + migration `20260915010000_expand_project_type` (additive `ALTER TABLE ... MODIFY COLUMN ... ENUM(...)`, same pattern as the prior `20260913171500_add_project_type_other`); `apps/api/src/shared/project.ts`'s `ProjectTypeEnum`; `apps/web/src/types/index.ts`'s `ProjectType`; `ProjectWizard.tsx`'s `PROJECT_TYPES` picker; `ProjectManagement.tsx`'s `PROJECT_TYPE_LABELS` (also drives its filter dropdown).
- **Verified live, not just typechecked:** applied the migration to `test_db` (raw SQL, same reasoning as every prior migration this session — avoids `prisma migrate deploy` picking up an unrelated pending migration from the concurrent session) and confirmed via `SHOW COLUMNS` that all 16 values are present. `POST /projects` with `project_type: "TOWNSHIP"` succeeds (201). In the real browser against the local API: `ProjectManagement.tsx`'s type filter lists all 14 new options plus both legacy labels; filtering to Township correctly shows only the new test project and hides existing APARTMENT-type ones. `project-workflow.test.ts` stays green. The four other project test files that fail (`project-amenities`/`-layout`/`-unit-features-activity`/`-units-pricing`) are the same pre-existing "Forbidden: Missing projects.update permission" `packages/shared`-vs-`apps/api` permission-constant drift already traced in item 1.1 — reconfirmed via zero uncommitted changes on all four files.
- **Not applied to production** — left for you to run yourself: `ALTER TABLE `project`MODIFY`project_type` ENUM('PLOTTED','APARTMENT','VILLA','INDEPENDENT_HOUSE','ROW_HOUSE','AGRICULTURAL_LAND','FARM_HOUSE','COMMERCIAL_SHOP','COMMERCIAL_OFFICE','MIXED_RESIDENTIAL','MIXED_USE','TOWNSHIP','GATED_COMMUNITY','MIXED','COMMERCIAL','OTHER') NULL;` (also saved as the migration file above).
