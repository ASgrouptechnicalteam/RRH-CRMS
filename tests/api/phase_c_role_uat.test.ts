...
describe('Phase C - Role UAT Beta Acceptance', () => {
  ...
  it('[ADMIN] PROJECTS_CREATE -> 201', async () => {
    const res = await request(app)
      ...
      .set('Authorization', `Bearer ${getToken(Roles.ADMIN)}`)
  ...
  it('[MARKETING_DIRECTOR] LEADS_CREATE -> 201', async () => {
    const res = await request(app)
      .post('/api/v1/leads')
      .set('Authorization', `Bearer ${getToken(Roles.MARKETING_DIRECTOR)}`)
  ...
  it('[PROJECT_MANAGER] PROJECTS_CREATE -> 201', async () => {
    const res = await request(app)
      .post('/api/v1/projects')
      .set('Authorization', `Bearer ${getToken(Roles.PROJECT_MANAGER)}`)
  ...
  it('[PROJECT_MANAGER] PROPERTIES_MD_APPROVE -> 403', async () => {
    const res = await request(app)
      .post('/api/v1/properties/999/md-approve')
      .set('Authorization', `Bearer ${getToken(Roles.PROJECT_MANAGER)}`)
      .send({ notes: 'test' });
    expect(res.status).toBe(403);
  });
  ...
  it('[DIGITAL_LEAD_OPERATOR] LEADS_CREATE -> 201', async () => {
    const res = await request(app)
      .post('/api/v1/leads')
      .set('Authorization', `Bearer ${getToken(Roles.DIGITAL_LEAD_OPERATOR)}`)
  ...
  it('[TELECALLER] LEADS_CREATE -> 201', async () => {
    const res = await request(app)
      .post('/api/v1/leads')
      .set('Authorization', `Bearer ${getToken(Roles.TELECALLER)}`)
  ...
  it('[DIGITAL_MARKETING_HEAD] PROPERTIES_MD_APPROVE -> 403', async () => {
    const res = await request(app)
      .post('/api/v1/properties/999/md-approve')
      .set('Authorization', `Bearer ${getToken(Roles.DIGITAL_MARKETING_HEAD)}`)
      .send({ notes: 'test' });
    expect(res.status).toBe(403);
  });
  ...
  it('[FINANCE] PAYMENTS_CREATE -> 201', async () => {
    const res = await request(app)
      ...
      .set('Authorization', `Bearer ${getToken(Roles.FINANCE)}`)
      .send({
  ...
  it('[AGENT] COMPLAINTS_CREATE -> 201', async () => {
    const res = await request(app)
      ...
      .set('Authorization', `Bearer ${getToken(Roles.AGENT)}`)
  ...
  it('[DIGITAL_MARKETING_EXECUTIVE] SITE_VISITS_READ -> 200', async () => {
    ...
    const res = await request(app)
      ...
      .set('Authorization', `Bearer ${getToken(Roles.DIGITAL_MARKETING_EXECUTIVE)}`);
  ...
  it('[SALES_MANAGER] TASKS_CREATE -> 201', async () => {
    const res = await request(app)
      .post('/api/v1/tasks')
      .set('Authorization', `Bearer ${getToken(Roles.SALES_MANAGER)}`)
      .send({
        title: 'SM Task',
        priority: 'HIGH',
        lead_id: baseLeadId,
        assignee_id: pmUserId,
